    ...
    m_close_log(config.log_close)
{
    ...
    LOG_INFO(BASE_TEXT + "WebServer created, http port = " + std::to_string(m_http_port));
    // std::cout << BASE_TEXT + "WebServer created, port = " << m_port << std::endl;
}
WebServer::~WebServer(){
    if (!m_shutdown_called) {
         LOG_WARN(BASE_TEXT + "~WebServer() 自动调用 shutdown()");
        shutdown();
    }
}

void WebServer::init(std::string& db_username, 
...
void WebServer::initSocket(){     // 创建 socket，绑定端口，listen，设置 socket 选项
...
void WebServer::eventLoop(){           // 启用服务器
...
void WebServer::handleSignalEvent() {
    int sig = 0;
...
void WebServer::refreshTimer(int fd)
...
void WebServer::addFd(int fd, uint32_t events)
{
    addFd(fd, events, true, m_trig_mode);
}

void WebServer::addFd(int fd, uint32_t events, bool one_shot, bool trig_mode)
{
    epoll_event ev;
    ev.data.fd = fd;
    ev.events = events;  
...
void WebServer::modFd(int fd, uint32_t events)
{
...
void WebServer::modFd(int fd, uint32_t events, bool one_shot, bool trig_mode)
{
    epoll_event ev;
    ev.data.fd = fd;
    ev.events = events;  
